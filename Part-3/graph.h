...
typedef struct readPattern {
	char pattern[Mpi];
	struct readPattern *next;
} PATTERN;
//4.Stucture declaration for FAULT
typedef struct saveFaults{
	char pattern[Mpi];
	LIST *outputLine[Mpo];
	struct saveFaults *next;
} FAULT; 
/***************************************************************************************************************************
...
void faultyCircuitSimulation(NODE *,int ,char *,int,char *);
void performLogicSimulation(NODE *,int ,int [2][2]);
int checkPrimaryip(int, int *,int);
int faultyGateInjection(int);
int gateNode(int);
int readTestPatterns(FILE *,PATTERN **);
void faultPatternSimulation(NODE *,char *,int,char *);
void savePattern(char *,FAULT **);
void saveTestSet(char *, PATTERN **);
void insertFaultList(FAULT **,int,char *,int);
void searchForFaultPresence(int,FAULT **, FAULT **,FAULT **);
void diagnosticResolution(FAULT **,int *);
int checkForAllPatterns(FAULT **,int);
int checkFault(LIST **, int);
void copyToNewList(FAULT **,LIST **,char *,int);
void updateNewList(FAULT **, FAULT **);
void deleteFault(LIST **,int);
